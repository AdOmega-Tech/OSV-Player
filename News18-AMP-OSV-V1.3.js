...
(function () {
  ...
  function _bindPodEvents(mgr, advanceFn) {
    ...
    mgr.addEventListener(google.ima.AdEvent.Type.CONTENT_RESUME_REQUESTED, function () {
      ...
      hideAdUI();
      // FIX 3: Guard against IMA internally unblocking the media element
      // between waterfall slots before _fireNext() can claim it.
  ...
  function onContentPause() {
    adPlaying = true;
    // FIX 1: Removed manual video.pause() — IMA already pauses the media
    // element internally via CONTENT_PAUSE_REQUESTED. Calling it again here
    // caused a double-pause race that produced the visible flicker.
  ...
  function fireMidroll(cue) {
    ...
    // FIX 2: Removed manual video.pause() — IMA fires CONTENT_PAUSE_REQUESTED
    // which handles the pause. Pausing here too created a stutter at every cue.
