...
(function () {
  ...
  ═══════════════════════════════════════════════════════════ */
  var CONTENT_MAP = {
    'tv9hindi.com'   : 'https://cdn.jwplayer.com/manifests/YCW7npTy.m3u8',
    'tv9telugu.com'  : 'https://cdn.jwplayer.com/manifests/YCW7npTy.m3u8',
    'tv9marathi.com' : 'https://cdn.jwplayer.com/manifests/YCW7npTy.m3u8',
    'tv9bangla.com'  : 'https://cdn.jwplayer.com/manifests/YCW7npTy.m3u8',
    'tv9kannada.com' : 'https://cdn.jwplayer.com/manifests/YCW7npTy.m3u8'
  };
  var DEFAULT_CONTENT_SRC = 'https://cdn.jwplayer.com/manifests/YCW7npTy.m3u8';
  ...
  /* ════ iOS WebKit DETECTION ══════════════════════════════════
     On iOS, ALL browsers (including Chrome) use WebKit under
     the hood (Apple mandate). WebKit only allows one active
     <video> element at a time, so IMA reuses the content
     <video> for ad playback instead of creating its own.
     Hiding that element produces a black screen during ads.
     We detect this condition and skip the visibility:hidden
     trick so the ad can render correctly.
  ═══════════════════════════════════════════════════════════ */
    ...
    function _fireNext() {
      ...
      ldr.addEventListener(
        ...
        function (e) {
          ...
          s.useStyledLinearAds    = false; // FIX: let IMA use native video element on iOS
          s.useStyledNonLinearAds = false; // FIX: same — avoids iframe overlay that blocks WebKit video
  ...
  function _bindPodEvents(mgr, advanceFn) {
    ...
    mgr.addEventListener(google.ima.AdEvent.Type.CONTENT_RESUME_REQUESTED, function () {
      ...
      hideAdUI();
      // FIX: On iOS WebKit, IMA reuses the content <video> element for ad
      // playback. Hiding it produces a black screen. Skip visibility:hidden
      // between pod slots on iOS; playContent() will make it visible again.
      if (!IOS) {
  ...
  function onContentPause() {
    adPlaying = true;
    // FIX: On iOS WebKit (incl. Chrome on iOS), all browsers share the same
    // WebKit engine (Apple mandate). WebKit only allows one active <video> at
    // a time, so IMA reuses our content <video> element for ad rendering
    // rather than creating a separate one. If we hide that element here, the
    // ad layer is hidden with it → black screen for the full ad duration.
    // Solution: skip visibility:hidden on iOS; the adContainer div sits above
    // the video in z-order and visually covers the content frame during the ad.
    if (!IOS) {
      video.style.visibility = 'hidden';
    }
  ...
  function playContent() {
    adPlaying = false;
    video.style.visibility = 'visible'; // always safe to restore — covers both iOS and non-iOS
