...
(function () {
    ...
    function _fireNext() {
      ...
      ldr.addEventListener(
        ...
        function (e) {
          ...
          if (!adcInitialized) {
            // ADC not yet initialized (no user tap yet on iOS).
            // Park the manager — initADC() will call init()+start() on first tap.
            pendingAdsManager = adsManager;
            return;
          }
  ...
  // Holds an adsManager that loaded before the user has tapped (iOS only).
  // Flushed and started inside initADC() once a gesture is available.
  var pendingAdsManager = null;
  ...
  function initADC() {
    ...
    // If an adsManager loaded before the first tap, start it now inside the gesture.
    if (pendingAdsManager) {
      var mgr = pendingAdsManager;
      pendingAdsManager = null;
      try {
        mgr.init(shellW(), shellH(), google.ima.ViewMode.NORMAL);
        mgr.start();
      } catch (ex) {
        console.warn('[IMA] deferred pod start() failed:', ex);
        // _fireNext is already bound via _bindPodEvents — the AD_ERROR handler
        // on the manager will advance the waterfall automatically.
      }
    }
  }
  /* ── Ad UI events (shared) ── */
  ...
  function onContentPause() {
    ...
    // iOS only allows one <video> to play at a time. Releasing the content
    // element's src frees the hardware decoder for IMA's ad video element,
    // which is why the creative shows a blank frame even though audio plays.
    if (isIOS) {
      video.pause();
      video.src = '';
      video.load();
  ...
  function playContent() {
    ...
    // On iOS we cleared video.src to free the decoder for the ad — restore it now.
    if (isIOS && !video.src) {
      video.src = CONTENT_SRC;
      video.load();
    }
  ...
  video.addEventListener('playing', function () {
    // On iOS (Safari and Chrome), muted autoplay can succeed without a tap.
    // The 'playing' event is the earliest safe point to call adDC.initialize()
    // in that case — the media element being active satisfies the gesture requirement.
    initADC();
